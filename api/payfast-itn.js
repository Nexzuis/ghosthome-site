// api/pf-init.js
// Clean, standalone endpoint for PayFast init (no Node 'crypto').
// Includes GET health so you can test it quickly.
...
function handler(req, res) {
  try {
    if (req.method === "GET") {
      // health check
      return res.status(200).json({ ok: true, route: "pf-init", hint: "POST from /pay to proceed" });
    }
    if (req.method !== "POST") {
    ...
    // Minimal, safe subscription fields
    const amount = "99.00";
    const fields = {
    ...
    return res.status(200).json({ ok: false, error: `pf-init crash: ${e?.message || e}` });
  }
}
