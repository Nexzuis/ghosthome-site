...
export default function handler(req, res) {
  try {
    ...
    req.on("end", () => {
      ...
      const posted = String(params.signature || "").trim().toLowerCase();
      const calc = md5(base).toLowerCase();
      const ok = posted === calc;

      console.log("pf-itn", { pf_payment_id: params.pf_payment_id, payment_status: params.payment_status, sigOk: ok });
