// /api/pf-init.cjs
// PayFast initiate (subscriptions) — builds fields + signature exactly per docs.
// Accepts GET (health) and POST (used by your /pay page).

function urlencodePhp(v) {
  return encodeURIComponent(String(v)).replace(/%20/g, "+"); // PHP-style: spaces => '+'
}

// Minimal dependency-free MD5 (ASCII-safe for our param strings)
function md5(s) {
...
function buildSignature(fields, passphrase) {
  // 1) include all non-empty fields (excluding 'signature')
  // 2) sort A→Z
  // 3) PHP urlencode keys/values (spaces => '+')
  // 4) append '&passphrase=...'
  // 5) MD5 hex
  const base =
    Object.keys(fields)
  ...
  // TEMP debugging (remove after you’re green)
...
function handler(req, res) {
  try {
    // Allow GET for quick health; your /pay page calls POST.
    ...
    // Minimal, spec-compliant subscription (R99 monthly)
    const amount = "99.00";
    const fields = {
      ...
      subscription_type: 1,     // subscriptions
      frequency: 3,             // monthly
      amount: amount,           // initial
      recurring_amount: amount, // recurring
