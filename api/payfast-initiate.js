...
// Node serverless + pure-JS MD5 (no 'crypto') + GET health check.
// Front-end stays the same (hidden form POST from /src/routes/pay.jsx).

function urlencodePhp(v) { return encodeURIComponent(String(v)).replace(/%20/g, "+"); }

// ---- tiny dependency-free MD5 (ASCII-safe) ----
function md5(input) {
  function rhex(n){const s="0123456789abcdef";let j,out="";for(j=0;j<4;j++)out+=s[(n>>(j*8+4))&15]+s[(n>>(j*8))&15];return out}
  ...
  function str2blks(str){const n=((str.length+8)>>6)+1;const blks=new Array(n*16).fill(0);
    ...
    blks[str.length>>2]|=0x80<<((str.length%4)*8); blks[n*16-2]=str.length*8; return blks}
  const x=str2blks(input); let a=1732584193,b=-271733879,c=-1732584194,d=271733878;
...
async function handler(req, res) {
  try {
    // Quick GET health so you can test without JS/POST
    if (req.method === "GET") {
      return res.status(200).json({ ok: true, route: "payfast-initiate", note: "Use POST from /pay page" });
    }
    if (req.method !== "POST") {
    ...
    if (!MERCHANT_ID || !MERCHANT_KEY || !PASSPHRASE || !RETURN_URL || !CANCEL_URL || !NOTIFY_URL) {
      return res.status(200).json({ ok: false, error: "Missing PayFast configuration" });
    }
    ...
    return res.status(200).json({ ok: false, error: `payfast-initiate (no-crypto) crash: ${e?.message || e}` });
  }
}
