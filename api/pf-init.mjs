// /api/pf-init.mjs  (same as your working file, only extra debug fields)
...
function rol(n,c){return(n<<c)|(n>>>(32-c))}function cmn(q,a,b,x,s,t){return add(rol(add(add(a,q),add(x,t)),s),b)}
...
function buildSignature(fields, passphrase){
  const base=Object.keys(fields).filter(k=>fields[k]!=null&&String(fields[k]).length>0).sort()
    .map(k=>`${urlencodePhp(k)}=${urlencodePhp(fields[k])}`).join("&")+`&passphrase=${urlencodePhp(passphrase)}`;
...
export default function handler(req,res){
  try{
    ...
    const engine=MODE==="live"?"https://www.payfast.co.za/eng/process":"https://sandbox.payfast.co.za/eng/process";

    const tail=PASSPHRASE.slice(-2);
    const resp={ok:true,engine,fields:{...fields,signature:sig}};
    if (req.method==="GET"){
      resp.debug_signature_base=base;
      resp.debug_signature_md5=sig;
      resp.debug_passphrase_len=PASSPHRASE.length;
      resp.debug_passphrase_tail=tail;
    }
